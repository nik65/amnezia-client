...
[[ -n "$AMNEZIA_BUILD_TESTS" ]]       && args+=("-DAMNEZIA_BUILD_TESTS=$AMNEZIA_BUILD_TESTS")
...
if [[ -n "$BUILD_AAB" ]]; then
    if [[ -n "$BUILD_PLAY" ]]; then
        run_traced cmake --build "$BUILD_PATH" --config "$CMAKE_BUILD_TYPE" -t "android_play_aab" --parallel "$BUILD_JOBS"
    else
        run_traced cmake --build "$BUILD_PATH" --config "$CMAKE_BUILD_TYPE" -t "aab" --parallel "$BUILD_JOBS"
    fi
fi
if [[ -n "$BUILD_APK" && -n "$BUILD_PLAY" ]]; then
    run_traced cmake --build "$BUILD_PATH" --config "$CMAKE_BUILD_TYPE" -t "android_play_apk" --parallel "$BUILD_JOBS"
fi
