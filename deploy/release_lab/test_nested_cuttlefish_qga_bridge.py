...
def boot_receipt(plan:InnerPlan)->dict:
    ...
    ar=[{"order":i,"path":path,"target_order":1 if i<3 else 2,"before_dev":1,"before_inode":11 if i<3 else 12,"after_dev":1,"after_inode":21 if i<3 else 22,"before_sha256":"2"*64,"before_size":90,"after_sha256":"3"*64,"after_size":100} for i,path in enumerate(paths,1)];adapter={"schema":2,"records":ar,"unique_target_writes":2,"alias":{"path":f"{plan.root}/runtime/assembly","target":f"{plan.root}/runtime/instance/assembly"},"before_identical":True,"after_identical":True,"source_shape":{"external_network_mode":"slirp","enable_modem_simulator":True,"ril_ipaddr":"","ril_gateway":"","ril_prefixlen":255,"ril_dns":""},"applied":{"ril_ipaddr":"10.0.2.15","ril_gateway":"10.0.2.2","ril_prefixlen":24,"ril_dns":"10.0.2.3"}}
...
def test_boot_probe_without_run_cvd_executes_adb_property_gate(tmp_path,capsys):
    ...
    after=[hashlib.sha256(q.read_bytes()).hexdigest() for q in config_paths];adapter={"schema":2,"records":[{"order":i,"path":str(q),"target_order":1 if i<3 else 2,"before_dev":1,"before_inode":11 if i<3 else 12,"after_dev":1,"after_inode":21 if i<3 else 22,"before_sha256":"2"*64,"before_size":90,"after_sha256":after[i-1],"after_size":q.stat().st_size} for i,q in enumerate(config_paths,1)],"unique_target_writes":2,"alias":{"path":str(root/"runtime/assembly"),"target":str(root/"runtime/instance/assembly")},"before_identical":True,"after_identical":True,"source_shape":{"external_network_mode":"slirp","enable_modem_simulator":True,"ril_ipaddr":"","ril_gateway":"","ril_prefixlen":255,"ril_dns":""},"applied":{"ril_ipaddr":"10.0.2.15","ril_gateway":"10.0.2.2","ril_prefixlen":24,"ril_dns":"10.0.2.3"}}
