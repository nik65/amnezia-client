...
def test_profile_is_pinned_for_arm_translation() -> None:
    ...
    assert profile["status"] == "ready-for-live-app-selfhosted-update"
    ...
    assert profile["network"]["fixture_guest_endpoint_env"] == "AMNEZIA_ANDROID_FIXTURE_GUEST_ENDPOINT"
...
def test_driver_is_scoped_and_fail_closed() -> None:
    ...
    assert "FIXTURE_GUEST_ENDPOINT" in source
    ...
    assert "app-selfhosted-update-pending" in source
    assert "diagnostic_fixture_download" in source
    assert "manifest-http.json" in source
    assert "candidate-http.apk" in source
    assert '"${WORK_HOME}/fixture-network.json"' in source
    assert "fixture network marker belongs to another run/profile" in source
    ...
    assert "monkey -p \"$PACKAGE\" 1" in update
...
def test_scenarios_keep_update_and_network_gates_separate() -> None:
    ...
    assert upgrade["status"] == "ready-for-live-app-selfhosted-update"
    assert upgrade["assertions"]["baseline"]["version_code"] == 2185
    assert upgrade["assertions"]["update"]["version_code"] == 2186
    assert upgrade["assertions"]["update"]["install_mode"] == "app-selfhosted-update-to-system-package-installer-ui"
...
def test_consumer_fixture_records_run_bound_hash_readback() -> None:
    ...
    assert "X-Amnezia-Sha256" in source
