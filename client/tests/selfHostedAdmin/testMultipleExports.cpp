...
private:
    ...
    ServerCredentials getCredentialsFromIni()
    {
        ...
        credentials.hostName = getEnvValue("SELF_HOSTED_SERVER_HOST_NAME");
        credentials.userName = getEnvValue("SELF_HOSTED_SERVER_USER_NAME");
        credentials.secretData = getEnvValue("SELF_HOSTED_SERVER_PASSWORD");
        credentials.port = getEnvValue("SELF_HOSTED_SERVER_SSH_PORT").toInt();

        logEnvValueState("SELF_HOSTED_SERVER_HOST_NAME");
        logEnvValueState("SELF_HOSTED_SERVER_USER_NAME");
        logEnvValueState("SELF_HOSTED_SERVER_PASSWORD");
        logEnvValueState("SELF_HOSTED_SERVER_SSH_PORT");
    ...
    int portForContainer(DockerContainer container) const
    {
        ...
        if (container == DockerContainer::Xray) {
            // The default 443 is occupied on the test VPS
            // (installContainer fails with ServerPortAlreadyAllocatedError).
            return 8443;
        }
    ...
    {
        ServerCredentials credentials = getCredentialsFromIni();

        if (credentials.hostName.isEmpty() || credentials.userName.isEmpty() || credentials.secretData.isEmpty()) {
            QSKIP("Set SELF_HOSTED_SERVER_HOST_NAME, SELF_HOSTED_SERVER_USER_NAME, SELF_HOSTED_SERVER_PASSWORD");
        }
...
private slots:
    ...
    void cleanupTestCase()
    {
        if (!m_serverId.isEmpty()) {
            ...
            /* Raises critical error because cant clear clientTable
