...
if(WIN32 AND (CONAN_NO_REMOTE OR "$ENV{CONAN_NO_REMOTE}" STREQUAL "1"))
    list(APPEND _CONAN_INSTALL_ARGS "-o=openssl/*:no_asm=True")
endif()

if(DEFINED ENV{AMNEZIA_BUILD_JOBS} AND NOT "$ENV{AMNEZIA_BUILD_JOBS}" STREQUAL "")
    string(STRIP "$ENV{AMNEZIA_BUILD_JOBS}" AMNEZIA_BUILD_JOBS_STRIPPED)
    if(NOT AMNEZIA_BUILD_JOBS_STRIPPED MATCHES "^[1-9][0-9]*$")
        message(FATAL_ERROR "AMNEZIA_BUILD_JOBS must be a positive integer")
    endif()
    list(APPEND _CONAN_INSTALL_ARGS "-c=tools.build:jobs=${AMNEZIA_BUILD_JOBS_STRIPPED}")
endif()

list(PREPEND _CONAN_INSTALL_ARGS "--build=missing")
if(CONAN_NO_REMOTE OR "$ENV{CONAN_NO_REMOTE}" STREQUAL "1")
    list(APPEND _CONAN_INSTALL_ARGS "-nr")
endif()
